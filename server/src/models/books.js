...
Books.search = async function (query = '', limit = 25, offset = 0) {
  let sqlQuery = `SELECT id, title, authors, genres, pages, rating, description, fts_tsv FROM books LIMIT ${limit} OFFSET ${offset}`;
  if (query.length >= 1) {
    sqlQuery = `SELECT * FROM (SELECT id, title, authors, genres, pages, rating, description, fts_tsv FROM books WHERE fts_tsv @@ plainto_tsquery('english', '${query}')) AS fts ORDER BY ts_rank_cd(fts.fts_tsv, plainto_tsquery('english', '${query}')) DESC LIMIT ${limit} OFFSET ${offset}`;
  }
